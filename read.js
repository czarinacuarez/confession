...
const confessionDB = firebase.database().ref("confessionwall");
...
function showModal(data) {
    ...
    document.getElementById('to-modal-content').textContent = "To: " + data.name;
    document.getElementById('course-modal-content').textContent = "" + data.program;
            ...
            entries.forEach(function (data) {
                parentElement.innerHTML += `
                    <a id="displayContainer" data-name="${data.name}" onclick="showModal('${encodeURIComponent(JSON.stringify(data))}')" class="block cursor-pointer bg-rose-300 max-w-2xl p-8 border border-gray-200 rounded-lg shadow overflow-hidden">
                        <h5 id="to-label" class="mb-2 text-xl text-white inline whitespace-normal break-all font-semibold">To: ${data.name}</h5>
